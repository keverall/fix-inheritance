...
.DESCRIPTION
    Public surface:
      Pure (no external dependencies, easily unit-tested):
        Get-LongPath, ConvertFrom-HResult, Get-ErrorReason,
        Resolve-OutputCsvPath, Get-ErrorRecordPath, Add-Failure
...
# Resolves the output CSV path. If the value already ends with .csv it
# is used verbatim; otherwise .csv is appended. Fixes the bug where
# passing -OutputPath "failures.csv" produced "failures.csv.csv".
function Resolve-OutputCsvPath {
    param([string]$Path)
    if ($Path -like '*.csv') { return [System.IO.Path]::GetFullPath($Path) }
    return [System.IO.Path]::GetFullPath("$Path.csv")
}
